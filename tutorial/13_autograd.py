...
# 학습 루프에서는 optimizer.zero_grad() 가 같은 일을 함
print("")
...
# numpy 변환 시 detach 가 필요한 이유 : autograd 그래프가 있으면 numpy 변환 불가
print("")

# ────────────── (5) no_grad — 추론 시 그래프 생성 안 함 ──────────────
print("[5] torch.no_grad")

x = torch.tensor(2.0, requires_grad=True)
with torch.no_grad():
    y = x ** 2
print(f"  no_grad 안의 y.requires_grad = {y.requires_grad}    (False 가 정상)")
# 추론(test.py) 에서 with torch.no_grad(): 로 감싸는 이유 — 메모리/속도 절약

# ────────────── 비교 정리 ──────────────
# - requires_grad=True : 이 tensor 가 관여한 모든 연산을 추적
# - .backward()        : 결과 tensor 부터 거꾸로 미분 (그래프 leaf 변수에 .grad 채움)
# - .grad              : 누적 — 매 step optimizer.zero_grad() 로 초기화 필요
